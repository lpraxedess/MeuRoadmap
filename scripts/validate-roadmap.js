...
const ids=new Map(),names=new Set(),warnings=[];
for(let pi=0;pi<R.phases.length;pi++){
 const p=R.phases[pi];if(!p.id||!p.name||!Array.isArray(p.tasks))throw Error(`Fase inválida: ${p.id||pi}`);
...
if(warnings.length)console.log(`WARN: ${warnings.length} competências usam evidência derivada do conteúdo.`);
