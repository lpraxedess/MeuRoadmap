...
function update(){
  ...
  document.querySelectorAll('.today-grid .task').forEach(function(row){
    ...
    small.textContent='';var bar=row.querySelector('.today-task-progress');
...
function unifyProgress(){
  ...
  var progress=panels[1];var old=focus.querySelector('.today-overall-progress');if(old)old.remove();
  ...
  var title=focus.querySelector('.focus-title');var label=focus.querySelector('.focus-label');
  if(title&&title.parentNode)title.parentNode.insertBefore(box,title.nextSibling);else if(label)focus.insertBefore(box,label);else focus.insertBefore(box,focus.firstChild);
  progress.remove();
}
